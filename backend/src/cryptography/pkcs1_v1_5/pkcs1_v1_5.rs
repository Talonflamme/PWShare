use crate::cryptography::rng::rng;
use rand::RngCore;

#[derive(Debug)]
pub struct PKCS1v1_5EncodingError {
    pub reason: &'static str,
}

#[derive(Debug)]
pub struct PKCS1v1_5DecodingError {
    pub reason: &'static str,
}

fn generate_random_pad_string(length: usize) -> Vec<u8> {
    let mut res = vec![0x00; length];

    // store a couple of randoms in advance to not have to compute them every iteration
    let mut random_cache: Vec<u8> = Vec::with_capacity(4);

    for byte in res.iter_mut() {
        while *byte == 0 {
            // no 0, since it's required as a separator

            if random_cache.len() == 0 {
                random_cache.extend_from_slice(&rng!().next_u32().to_be_bytes());
            }

            *byte = random_cache.pop().unwrap();
        }
    }

    res
}

/// The padding and unpadding differs for encryption and signing. This enum must be passed
/// to those methods to distinguish the cases.
#[derive(Copy, Clone, Debug)]
pub enum PKCS1v1_5Mode {
    /// When encoding, the block type is 0x02 and the pad string are random non-zero bytes
    Encryption,
    /// When signing, the block type is 0x01 and the pad string are all 0xFF.
    Signature,
}

/// Pads a message using the PKCS#1 v1.5 padding specification.
///
/// # Arguments
///  * `message´ - A byte slice representing the message to be padded of arbitrary length
/// * `k` - The length in bytes of the RSA modulus (size of `n` in bytes)
/// * `mode` - if this padding is for encryption or signing.
///
/// # Returns
///
/// A `Result` containing the padded message as a `Vec<u8` on success, or an
/// `PKCS1v1_5EncodingError` on failure.
///
/// # Errors
///
/// Returns `OAEPEncodingError` if the message is too long for the given `k`.
pub fn pad(
    message: &[u8],
    k: usize,
    mode: PKCS1v1_5Mode,
) -> Result<Vec<u8>, PKCS1v1_5EncodingError> {
    // Need 2 marker bytes, 1 separator and at least 8 random bytes (PS)
    if message.len() > k - 11 {
        return Err(PKCS1v1_5EncodingError {
            reason: "Message too large",
        });
    }

    let mut encoded_message = Vec::with_capacity(k);

    // Start with 0x00
    encoded_message.push(0x00);

    match mode {
        PKCS1v1_5Mode::Encryption => {
            // Block Type, 0x02 for encryption
            encoded_message.push(0x02);
            // Pad String (PS), random non-zero bytes for encryption
            let mut pad_string = generate_random_pad_string(k - message.len() - 3);
            encoded_message.append(&mut pad_string);
        }
        PKCS1v1_5Mode::Signature => {
            // Block Type, 0x01 for signing
            encoded_message.push(0x01);
            // Pad String (PS), 0xFF bytes for signing
            let mut pad_string = vec![0xFFu8; k - message.len() - 3];
            encoded_message.append(&mut pad_string);
        }
    }

    // Separator between padding and data
    encoded_message.push(0x00);

    // actual data
    encoded_message.extend_from_slice(message);

    Ok(encoded_message)
}

/// Removes PKCS#1 v1.5 padding from a message.
///
/// # Arguments
///
/// * `padded_message` - A byte slice representing the PKCS#1 v1.5 padded message.
/// * `k` - The length in bytes of the RSA modulus (size of `n` in bytes)
/// * `mode` - If this operation is for signing or encryption/decryption.
///
/// # Returns
///
/// A `Result` containing the original unpadded message as a `Vec<u8` on success, or an
/// `PKCS1v1_5DecodingError` on failure.
///
/// # Errors
///
/// Returns `PKCS1v1_5DecodingError` if the padding is invalid or the message cannot be recovered.
pub fn unpad(
    padded_message: &[u8],
    k: usize,
    mode: PKCS1v1_5Mode,
) -> Result<Vec<u8>, PKCS1v1_5DecodingError> {
    if padded_message.len() != k {
        return Err(PKCS1v1_5DecodingError {
            reason: "len(EM) != k",
        });
    }

    if padded_message[0] != 0x00 {
        return Err(PKCS1v1_5DecodingError {
            reason: "First byte of EM must be 0x00",
        });
    }

    match padded_message[1] {
        0x02 => {
            if !matches!(mode, PKCS1v1_5Mode::Encryption) {
                return Err(PKCS1v1_5DecodingError {
                    reason: "Block Type 0x02 for signing, 0x01 expected",
                });
            }
        }
        0x01 => {
            if !matches!(mode, PKCS1v1_5Mode::Signature) {
                return Err(PKCS1v1_5DecodingError {
                    reason: "Block Type 0x01 for encryption, 0x02 expected",
                });
            }
        }
        _ => {
            return Err(PKCS1v1_5DecodingError {
                reason: "Block Type must be 0x02 for encryption and 0x01 for signing",
            })
        }
    }

    let separator_index = 2 + padded_message
        .iter()
        .skip(2)
        .position(|byte| *byte == 0x00)
        .ok_or(PKCS1v1_5DecodingError {
            reason: "No separator 0x00 found",
        })?;

    // for signature, all PS bytes must be 0xFF
    if matches!(mode, PKCS1v1_5Mode::Signature) {
        if padded_message
            .iter()
            .take(separator_index)
            .skip(2)
            .any(|&b| b != 0xff)
        {
            return Err(PKCS1v1_5DecodingError {
                reason: "non 0xFF byte in PS found",
            });
        }
    }

    if separator_index < 10 {
        return Err(PKCS1v1_5DecodingError {
            reason: "len(PS) < 8",
        });
    }

    let message = &padded_message[separator_index + 1..];

    Ok(message.to_vec())
}

#[cfg(test)]
mod tests {
    use super::*;

    fn from_hex(hex: &str) -> Vec<u8> {
        (0..hex.len())
            .step_by(2)
            .map(|i| u8::from_str_radix(&hex[i..=i + 1], 16).unwrap())
            .collect()
    }

    fn test_case_unpad(em: &str, k: usize, expected: &str, mode: PKCS1v1_5Mode) {
        assert_eq!(
            unpad(from_hex(em).as_slice(), k, mode).unwrap(),
            from_hex(expected)
        );
    }

    #[test]
    fn test_unpad_encrypt() {
        test_case_unpad("", 256, "96946c9765d1a26ab024d317182a969be0763cd4ae4b54dad83f56f07fa95ed8fb00558eb87ab6fb97a75d8fb4e8bb4fb309629698adba6f4fc2d6d7c22cede105a349fa0855115ab68571afa261fba6e26bf7b07bae4cf25be40984472bc1522163c778777066eccf6c", PKCS1v1_5Mode::Encryption);
        test_case_unpad("", 256, "", PKCS1v1_5Mode::Encryption);
        test_case_unpad("", 256, "5fb95586ca8e2061a130695bba0c44f1772f6ce8bc2fd0b80c9ce9c1a47f2f936e5a4c140fe696bedcacb8bcb1dfba69acf20cf6a02f5d174c2d3944093b112fa3e43c7b23210f2ed45aa750ef4e72ee3a1ae29f08cb1e6c27e23e4347216c1a43428d16d43934781594a5d33d703e1e4423fb0d", PKCS1v1_5Mode::Encryption);
        test_case_unpad("", 256, "17f25dafcb1187dda30f2a83d07f2f0334ceabd2f701b1c7a03698e545e286846ec02141c4098dda75c820a5eeddf75bd9d97726aa03a92172bb6809df460c6eee1564455b2360f77ab3ef81b784e91711261e34dd4e3d8041b7a54cd300be", PKCS1v1_5Mode::Encryption);
        test_case_unpad("", 256, "", PKCS1v1_5Mode::Encryption);
    }

    #[test]
    fn test_unpad_signature() {
        test_case_unpad("", 256, "", PKCS1v1_5Mode::Signature);
        test_case_unpad("", 256, "", PKCS1v1_5Mode::Signature);
        test_case_unpad("", 256, "261af6daf82e983bb2dfd1ddd03adb19d7f34f6147", PKCS1v1_5Mode::Signature);
        test_case_unpad("", 256, "2f37bca2de4288d541a4655de9b2a329082ec45e5305f35aae40a235a49990ab85a85844fe7eaab34998c8efd8fafcaeaba5e5a5576f47127302f09be3ef8da201becf2b0764bd88d40da8bacbedab3eefbed42972af5676f7fd2361f34c1b45e66651fda26fe6ec2806d7a3c4612bb03942948e38", PKCS1v1_5Mode::Signature);
    }

    fn test_case_pad(message: &str, k: usize, mode: PKCS1v1_5Mode) {
        let m = from_hex(message);
        let em = pad(m.as_slice(), k, mode).unwrap();
        let dm = unpad(em.as_slice(), k, mode).unwrap();
        assert_eq!(m, dm);
    }

    #[test]
    fn test_pad_encrypt() {
        // since we know that unpad works correctly (see above test), we can simply call pad and unpad and check if it matches
        test_case_pad("", 256, PKCS1v1_5Mode::Encryption);
        test_case_pad(
            "5fdd71fffd492cefa752b02eb4b4",
            256,
            PKCS1v1_5Mode::Encryption,
        );
        test_case_pad("", 256, PKCS1v1_5Mode::Encryption);
        test_case_pad("913a42ce70f0c9c58e01fd7649318b028a070d05a036d7eac3e839edc877d920e243c95a6c32bdc46e7f154f70299179ac4ae40fe4c4ca0c50fdbbee82d34a91ec761db015c039fafdfa55415c4f7f3713269a90b5adb5975786b6e408bdd5f43f7e827fd9a5e3b2bbd5fd34cac7dfe3", 256, PKCS1v1_5Mode::Encryption);
        test_case_pad("", 256, PKCS1v1_5Mode::Encryption);
    }

    #[test]
    fn test_pad_signature() {
        test_case_pad("f9ae0c558069103b34141bfee561812aa1d6e9e6f8fcf0e958a1b862c7cff393b9477be5b888acb0e0af9048504e958d0776db469bf12f6258461ada5cb94bbbad9264c37cc8909345556302a059fdc7981ac33fc75e5da96bb372ff3b3cdcd6b104", 256, PKCS1v1_5Mode::Signature);
        test_case_pad("afd9e4f29d51b1270a1796fc16975b263160dffac0a521b9ab64a25188fce12a5061ec41ac7b97a7d2b5f2b00302eb9e4a16739507563b0f8b70ac1e76d2c8c8832777969218730840b4be6e4f9decdd3868264826c84ed7d8d31dbcd5794337a59c9d1284bbeab99570365c41a47abb79", 256, PKCS1v1_5Mode::Signature);
        test_case_pad(
            "1034248e96fe81abf7e4be04703baf1ff5aa431980e3c8e97438c2248d9258b0e6",
            256,
            PKCS1v1_5Mode::Signature,
        );
        test_case_pad("811cb40dec653d44075b5b518d67ae39e0c2b19ec6eaac060a6fb39b9bf5e993af7a9d2e2167556f3a9245e3eecb", 256, PKCS1v1_5Mode::Signature);
    }

    #[test]
    #[should_panic(expected = "too large")]
    fn test_pad_encrypt_message_too_large() {
        pad(from_hex(
            ""
        ).as_slice(), 256, PKCS1v1_5Mode::Encryption).unwrap();
    }

    #[test]
    #[should_panic(expected = "len(EM) != k")]
    fn test_unpad_encrypt_len_missmatch() {
        unpad(
            from_hex("00023859ef1c21e0cb6ae0af7a501343cf0d21a780828290eabbe43f9a9dac6584c6c7382eec049dcc88daf493c87ee79de4c613af2a48af91463586acd75ae1d03fb95e6afe836c8f4d2c2607e8601db6d32c73e758cc51361fd35858ab856b6a8be3cc44d0bf0a9db387f2c17d8fdc434071a5ab6a91aec339ff7d66b998").as_slice(),
            128,
            PKCS1v1_5Mode::Encryption
        ).unwrap();
    }

    #[test]
    #[should_panic(expected = "non 0xFF")]
    fn test_unpad_signature_ps_missmatch() {
        unpad(
            from_hex(
                ""
            ).as_slice(), 256, PKCS1v1_5Mode::Signature).unwrap();
    }
}
