use pwshare_macros::{generate_k_md5, generate_k_sha256, generate_k_sha512};
...
macro_rules! copy_chunk_into_words_le {
    ($bytes:ident, $words:ident, $ty: ident) => {
        for (i, chunk) in $bytes.chunks_exact(($ty::BITS / 8) as usize).enumerate() {
            $words[i] = $ty::from_le_bytes(chunk.try_into().unwrap());
        }
    };
}
...
impl HashFunction for Md5 {
    ...
    fn hash(&self, message: &[u8]) -> Vec<u8> {
        ...
        for chunk in message.chunks(64) {
            ...
            copy_chunk_into_words_le!(chunk, m, u32);
        ...
        res.extend_from_slice(&c0.to_le_bytes());
        res.extend_from_slice(&d0.to_le_bytes());
...
impl HashFunction for Sha1 {
    ...
    fn hash(&self, message: &[u8]) -> Vec<u8> {
        ...
        for chunk in message.chunks_exact(64) {
            ...
            copy_chunk_into_words_be!(chunk, w, u32);
...
const K_256: [u32; 64] = generate_k_sha256!();

const K_512: [u64; 80] = generate_k_sha512!();
...
macro_rules! sha512 {
    ($message:ident, $h0:ident, $h1:ident, $h2:ident, $h3:ident, $h4:ident, $h5:ident, $h6:ident, $h7:ident) => {
        for chunk in $message.chunks_exact(128) {
            ...
            copy_chunk_into_words_be!(chunk, w, u64);
...
mod tests {
    ...
    #[test]
    fn const_md5() {
        let k_md5 = generate_k_md5!();
        // just do a couple of tests
        assert_eq!(k_md5.len(), 64);
        assert_eq!(k_md5[0], 0xd76aa478);
        assert_eq!(k_md5[10], 0xffff5bb1);
        assert_eq!(k_md5[40], 0x289b7ec6);
        assert_eq!(k_md5[50], 0xab9423a7);
        assert_eq!(k_md5[63], 0xeb86d391);
    }

    #[test]
    fn const_sha256() {
        let k_sha256 = generate_k_sha256!();

        assert_eq!(k_sha256.len(), 64);
        assert_eq!(k_sha256[0], 0x428a2f98);
        assert_eq!(k_sha256[10], 0x243185be);
        assert_eq!(k_sha256[31], 0x14292967);
        assert_eq!(k_sha256[50], 0x2748774c);
        assert_eq!(k_sha256[63], 0xc67178f2);
    }

    #[test]
    fn const_sha512() {
        let k_sha512 = generate_k_sha512!();

        assert_eq!(k_sha512.len(), 80);
        assert_eq!(k_sha512[0], 0x428a2f98d728ae22);
        assert_eq!(k_sha512[4], 0x3956c25bf348b538);
        assert_eq!(k_sha512[30], 0x06ca6351e003826f);
        assert_eq!(k_sha512[50], 0x2748774cdf8eeb99);
        assert_eq!(k_sha512[79], 0x6c44198c4a475817);
    }
    ...
    fn md5() {
        ...
        assert_eq!(Md5.h_len() * 8, 128);
        assert_eq!(Md5.block_size_bytes() * 8, 512);
    }
